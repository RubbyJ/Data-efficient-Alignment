...
class NeuMatch_YMS_BERT_Dataset(data.Dataset):
    def __init__(self, root_dir, ds_name, max_seq_len, phase, random_project):
        super(NeuMatch_YMS_BERT_Dataset, self).__init__()
        ...
        self.vid_embedding = np.load(os.path.join(path, 'fasterrcnn_features/video', 'one_norm_' + 'rp_' * random_project +
                                                  'full_' * (1 - random_project) + 'fasterrcnn_video.npy'),
                                     allow_pickle=True).item()
        self.bert_embedding = np.load(os.path.join(path, 'fasterrcnn_features/sentence', 'one_norm_' + 'rp_' * random_project +
                                                   'full_' * (1 - random_project) + 'bert.npy'),
                                      allow_pickle=True).item()
